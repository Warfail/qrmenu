...
async function main() {
  ...
  const okSrv = srvUri ? await tryConnect("SRV (.env.local)", srvUri) : true;
  const okDirect = await tryConnect("NON-SRV fallback", NON_SRV_URI);

  if (okSrv || okDirect) {
    console.log("\nRESULT: Database reachable.");
  } else {
    console.error("\nRESULT: Both connection attempts failed.");
    process.exit(1);
  }
}
